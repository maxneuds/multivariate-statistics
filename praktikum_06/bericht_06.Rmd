...
Einmal der gesamte Datensatz.

```{r}
library(ggplot2)
library(tidyr)
ggdf = gather(df, key='key', value='value', c(3,4,5,6,7,8,9,10))
gg = ggplot(
  data = ggdf,
  mapping = aes(
    x = ID,
...
Nun beschränkt auf `DBP` als $X_1$ und `SBP` als $X_2$.

```{r}
df = data.frame(DBP = df$DBP, SBP = df$SBP)
ggdf = gather(df, key='key', value='value')
gg = ggplot(
  data = df,
  mapping = aes(
    x = DBP,
...
x1 = df$DBP
x2 = df$SBP
sol = round(cor(x1, x2), 3)
sol
```

Wir erhalten eine Korrelation von $`r sol`$ und somit sind DBP und SBP positiv korreliert.
