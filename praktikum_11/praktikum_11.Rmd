...
```
Der Datensatz weist keine Missings auf. 


```{r}
#die e Zahlen umgewandelt
clust <- format(clust, scientific = FALSE)

colnames(clust) <- c("Column1","Column2", "Column3","Column4")
...
clust[c(1:4)] <- lapply(clust[c(1:4)], as.numeric)
str(clust)
...
scaled_data = as.matrix(scale(clust))
...
kmm = kmeans(scaled_data,3,nstart = 60,iter.max = 15) #we keep number of iter.max=15 to ensure the algorithm converges and nstart=50 to #ensure that atleat 50 random sets are choosen  
kmm
